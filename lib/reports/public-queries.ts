...
// the marketing site since it only exposes name/location/a computed
// charity figure, never raw revenue or sales rows (see migration
// 004_public_landing_data.sql).
export async function fetchLocationImpact(supabase: SupabaseServerClient): Promise<LocationImpact[]> {
  const { data } = await supabase
    ...
    .select("id, name, location, charity_estimate")
