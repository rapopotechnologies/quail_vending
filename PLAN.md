...
8. **Inventory management & UI/UX pass 2** — follow-on ideas surfaced after Phase 7, split by whether they need a product/infra decision first.
   1. ✅ **Restock suggested quantity** — `par_level` now threaded through `RawSlot`/`RestockSlot` (`app/(protected)/admin/restock/page.tsx`, `components/restock/restock-form.tsx`); qty-added inputs default to `par_level - current_qty` (floored at 0) and show a "Suggested: N (to par M)" caption per slot.
   2. ✅ **Bulk par-level edit** — vendored `components/ui/checkbox.tsx` (`@radix-ui/react-checkbox`, first checkbox in the codebase), added row-selection state to `products-view.tsx`, select-all/per-row checkboxes to `products-table.tsx`, and a `BulkParLevelDialog` bulk-action bar backed by a new `bulkUpdateParLevel(ids: string[], parLevel: number)` action in `app/actions/products.ts`.
   3. ✅ **Low-stock notifications** — went with in-app only (no `resend`/email dependency added). `app/(protected)/admin/layout.tsx` fetches `fetchLowStockSlots` + `fetchLowBulkStockProducts` and passes a combined count into `Navbar` (`components/layout/navbar.tsx`), which renders a destructive-variant count badge next to the "Dashboard" link whenever anything's at/under par — visible on every admin page, not just when someone happens to open the dashboard. Email digest stays a documented option if in-app turns out to be insufficient once real usage shows whether staff check it.
   4. ⏸️ **Mobile on-device verification** — still blocked on the same constraint noted in Phases 6/7: this dev environment can't produce a true narrow-viewport browser render. Needs the user testing on an actual phone, or a screenshot handoff, to move past code-level review.
   5. ✅ **Expiry/freshness tracking** — full batch/lot tracking (user confirmed this matters). New `product_lots` table (migration `011_product_expiry_lots.sql`, see CLAUDE.md) records one lot per purchase with its own `qty`/`expiry_date`; `RecordPurchaseDialog` now requires an expiry date. `createRestockEvent` draws lots down oldest-expiry-first (FIFO) in step with the existing `warehouse_qty` drawdown. New "Expiring soon" card on Dashboard + Reports (`ExpiringSoonList`, `fetchExpiringLots`, 30-day window), folded into the navbar low-stock badge count. Pre-migration `warehouse_qty` has no backing lot and stays untracked for expiry until replaced by a new purchase — a one-time gap, not an ongoing one.
   6. ✅ **Depletion-rate signal** — `velocityByProduct` (`lib/reports/aggregate.ts`) computes units/day per product from whatever sales range is already selected on `/admin/reports`; `fetchTotalOnHandByProduct` (`lib/reports/queries.ts`) reuses the same warehouse-qty + in-machines total the Products page computes. Combined into a "Depletion forecast" card (`components/reports/depletion-forecast.tsx`) showing estimated days-left per product, most urgent first, red-flagged at ≤14 days. Products with no sales in the selected range are excluded (no rate to project from) rather than shown as infinite.
   7. ✅ **Crate/loose-item simplification** — user flagged the bulk-stock model as over-engineered relative to how staff actually think about it (crates purchased/in stock, add by crate or rarely by loose item, subtract per item, crates deplete automatically). Confirmed no schema change needed for the crate/loose split itself — `warehouse_qty` (one running total) + `units_per_case` already fully determine it; `lib/inventory.ts`'s `formatAsCrates` derives "N crates + M loose" for display (`ProductsTable`'s bulk-stock and total-on-hand columns). The one real UI gap fixed: `RecordPurchaseDialog` previously forced case-multiplied purchases whenever `units_per_case > 1` with no way to add loose items; it now has an explicit Crates/Individual-items tab. `product_lots` (item 5 above) stayed as-is — a purchase transaction (potentially several crates bought together, sharing one expiry date) maps to exactly one lot, so no extra state was needed there either.
   8. ✅ **Subtraction-as-sale, then simplified further to plain stock removal** — first pass added `logSlotSale` (decrement stock + insert a `sales` row, since there's no POS API from the vending hardware's own software, Micromart, to reconcile against). User then clarified they don't care about sales/revenue tracking at all for inventory purposes — just purchases, what's left, and expiry. Renamed/simplified to `removeSlotStock` (`app/actions/machine-slots.ts`): decrements `stock_levels.current_qty` only, no price, no `sales` row. Exposed via a "Remove stock" quick action per slot on the machine detail page (`components/machines/remove-stock-dialog.tsx`, wired into `SlotsTable`). The existing `/admin/sales` manual-entry page and dashboard/reports revenue views were explicitly left alone — untouched, unrelated feature for whoever still wants revenue figures. See CLAUDE.md's "Inventory consumption vs. sales tracking" section.
