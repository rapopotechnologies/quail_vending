...
7. 🔄 **Admin UI/UX pass** — bulk stock/machine images and addresses (✅ done, see CLAUDE.md migration 010) surfaced a broader ask: optimize the day-to-day staff/admin experience before adding more features. In priority order:
   1. **Themed delete-confirmation dialog** — `DeleteConfirmButton` currently calls `window.confirm()`, which looks out of place next to the rest of the UI and fully blocks the page while open. Replace with a vendored Radix `AlertDialog` (`components/ui/alert-dialog.tsx`) styled to match `dialog.tsx`.
   2. **Staff invite flow in-app** — today every invite requires the Supabase Dashboard. Add a `super_admin`-gated `/admin/team` page: list `profiles` + email, invite by email via `lib/supabase/service.ts`'s admin API (`auth.admin.inviteUserByEmail`), matching the existing invite → `/auth/callback` → `/set-password` flow already built in Phase 1.
   3. **Machines table search** — bring `/admin/machines` to parity with `/admin/products` (name search at minimum; category filter doesn't apply here).
   4. **Sortable table columns** — Products, Machines, Sales, Reports currently return whatever order the query gives; add click-to-sort on the columns staff actually scan (name, status, total on hand, date).
   5. **Reports/dashboard date-range scoping** — both currently fetch full sales history on every load with no way to narrow it; add a date-range control so this doesn't degrade as sales volume grows.
   6. **Loading states** — Reports in particular can take a moment with full history; add a skeleton/spinner instead of a blank page during the fetch.
   7. **Real mobile device check** — Phase 6's mobile pass was code-level only (dev browser automation can't produce a true narrow viewport); verify nav + dialogs on an actual phone once the above lands.
