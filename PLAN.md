...
8. **Inventory management & UI/UX pass 2** — follow-on ideas surfaced after Phase 7, split by whether they need a product/infra decision first.
   ...
   8. ✅ **Subtraction-as-sale** — no POS API from the vending hardware's own software (Micromart) yet, so no automated sales feed to reconcile against; user wants item subtraction and sales logging to be the *same* action rather than double entry. New `logSlotSale(machineId, slotId, qty)` (`app/actions/sales.ts`) decrements `stock_levels.current_qty` and inserts a `sales` row (unit_price snapshotted from the product's `sell_price`) in one step, exposed via a "Log sale" quick action per slot on the machine detail page (`components/machines/log-sale-dialog.tsx`, wired into `SlotsTable`). The existing free-form `/admin/sales` manual-entry page stays as a fallback for cases that don't fit the slot-subtraction model (backfilling/correcting an entry, a sale outside the normal visit flow) — see CLAUDE.md's new "Sales tracking" section for the full split.
